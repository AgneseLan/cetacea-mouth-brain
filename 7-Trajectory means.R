...
#Calculate average PC proportion between the 2 modules - to use for both groups as indicative value
PC1_traj_dev <- c(round(as.numeric(trajectory_means_groups_rostrum[["pca"]][["sdev"]][1]^2/sum(trajectory_means_groups_rostrum[["pca"]][["sdev"]]^2)*100), digits = 2),
round(as.numeric(trajectory_means_groups_braincase[["pca"]][["sdev"]][1]^2/sum(trajectory_means_groups_braincase[["pca"]][["sdev"]]^2)*100), digits = 2))
PC2_traj_dev <- c(round(as.numeric(trajectory_means_groups_rostrum[["pca"]][["sdev"]][2]^2/sum(trajectory_means_groups_rostrum[["pca"]][["sdev"]]^2)*100), digits = 2),
                  round(as.numeric(trajectory_means_groups_braincase[["pca"]][["sdev"]][2]^2/sum(trajectory_means_groups_braincase[["pca"]][["sdev"]]^2)*100), digits = 2))
mean_PC1_traj <- mean(PC1_traj_dev)
mean_PC2_traj <- mean(PC2_traj_dev)
...
trajectory_means_groups_modules_myst_ggplot <- ggplot(trajectory_means_groups_modules_pcscores_myst, aes(x = PC1, y = PC2, shape = module, group = module))+
  ...
  scale_linewidth_manual(values = c(0.8, 1.2, 1.5, 1.8))+
  theme_bw()+
  xlab(paste0("PC 1 (",mean_PC1_traj,"%)"))+ 
  ylab(paste0("PC 2 (",mean_PC2_traj,"%)"))+
...
#perform cor.test "kendall" or "spearman" to understand correlation and calculate p.value for differences in pcscores between rostrum and braincase each group
...
fit_shape_means_modules_2_myst_group_category <- procD.lm(as.matrix(phylo_pcscores_means_R_B_mysticeti[,c(1:14)]) ~ phylo_pcscores_means_R_B_mysticeti$module * phylo_pcscores_means_R_B_mysticeti$category, iter = 999, RRPP = F)

#Check that there is a significant correlation
summary(fit_shape_means_modules_2_myst_group_category)

#Save results to file
sink("Output/7-Trajectory means/fit_shape_means_modules_2_myst_group_category.txt")
summary(fit_shape_means_modules_2_myst_group_category)
sink() 

#Use fit to calculate trajectories
trajectory_means_groups_modules_2_myst <- trajectory.analysis(fit_shape_means_modules_2_myst_group_category, groups = phylo_pcscores_means_R_B_mysticeti$module, traj.pts = phylo_pcscores_means_R_B_mysticeti$category, 
                                                              pca = TRUE, print.progress = F) 

#View results
#Magnitude differences between trajectories, standard summary - are trajectories different in length?
trajectory_means_groups_modules_2_myst_MD <- summary(trajectory_means_groups_modules_2_myst, show.trajectories = TRUE, attribute = "MD") 
trajectory_means_groups_modules_2_myst_MD

#Trajectory correlations -  are trajectories different in angle/direction?
trajectory_means_groups_modules_2_myst_TC <- summary(trajectory_means_groups_modules_2_myst, show.trajectories = TRUE, attribute = "TC", angle.type = "deg")
trajectory_means_groups_modules_2_myst_TC 

#Trajectory shape differences - are trajectories different in shape?
trajectory_means_groups_modules_2_myst_SD <- summary(trajectory_means_groups_modules_2_myst, show.trajectories = TRUE, attribute = "SD") 
trajectory_means_groups_modules_2_myst_SD 

#Save results to file
sink("Output/7-Trajectory means/trajectory_means_groups_modules_2_mysticeti.txt")
print("Magnitude difference (absolute difference between path distances) - length")
trajectory_means_groups_modules_2_myst_MD 
print("Correlations (angles) between trajectories - direction")
trajectory_means_groups_modules_2_myst_TC
...
print("Summary tables")
trajectory_means_groups_modules_2_myst_MD$summary.table
trajectory_means_groups_modules_2_myst_TC$summary.table
trajectory_means_groups_modules_2_myst_SD$summary.table
sink() 

#Plot results - PCA of fitted values
trajectory_means_groups_modules_2_myst_plot <- plot(trajectory_means_groups_modules_2_myst, main = "Trajectories of growth mean shapes by modules",  pch = c(22,24), #title and type of point to be used
                                                    col = c("darkgray","lightgray"), bg = c("darkgray","lightgray"), cex = 1, font.main = 2) #improve graphics
#Add line between groups_rostrum
add.trajectories(trajectory_means_groups_modules_2_myst_plot, 
...
##Make better PCA plot using ggplot
#Read PC scores as tibble
trajectory_means_groups_modules_2_myst_pcscores <- as_tibble(trajectory_means_groups_modules_2_myst_plot [["pc.points"]])

#Add group names and other attributes to tibble as columns
trajectory_means_groups_modules_2_myst_pcscores <- trajectory_means_groups_modules_2_myst_pcscores %>% mutate(specimens = phylo_pcscores_means_R_B_mysticeti$specimens, category = phylo_pcscores_means_R_B_mysticeti$category,  
                                                                                                              module = phylo_pcscores_means_R_B_mysticeti$module, size = phylo_pcscores_means_R_B_mysticeti$size)
glimpse(trajectory_means_groups_modules_2_myst_pcscores)

#Calculate means of PC1 and PC2 at each category per group to draw trajectories
trajectory_means_groups_modules_2_myst_pcscores_means <- trajectory_means_groups_modules_2_myst_pcscores %>% group_by(module, category) %>%
  summarise_at(vars(PC1, PC2), list(mean = mean))              #function for means, both columns
glimpse(trajectory_means_groups_modules_2_myst_pcscores_means)

#Rename columns so they are easier to use for plot
trajectory_means_groups_modules_2_myst_pcscores_means <- trajectory_means_groups_modules_2_myst_pcscores_means %>% rename(x = PC1_mean, y = PC2_mean)
glimpse(trajectory_means_groups_modules_2_myst_pcscores_means)
...
#Nice plot
trajectory_means_groups_modules_2_myst_ggplot <- ggplot(trajectory_means_groups_modules_2_myst_pcscores, aes(x = PC1, y = PC2, shape = module, group = module))+
  geom_point(aes(alpha = category), size = 2, colour = "grey10", fill = "grey10", show.legend = F)+
  geom_point(data = trajectory_means_groups_modules_2_myst_pcscores_means, aes(x = x, y = y, fill = module, shape = module, alpha = category, group = module), colour = "grey10",
             size = 6, inherit.aes = F)+
  geom_path(data = trajectory_means_groups_modules_2_myst_pcscores_means, aes(x = x, y = y, colour = module, group = module, linewidth = category), inherit.aes = F,
            linejoin = 'mitre', arrow = arrow(angle = 40, length = unit(0.02, "npc"), ends = "last", type = "open"), show.legend = F)+
  scale_alpha_manual(name = "Growth stage", labels =  levels(categories), #to be ordered as they appear in tibble
                     values = c(0.3, 0.4, 0.6, 0.8))+            #legend and color adjustments
  ...
  scale_linewidth_manual(values = c(0.8, 1.2, 1.5, 1.8))+
  theme_bw()+
  xlab(paste0("PC 1 (",round(as.numeric(trajectory_means_groups_modules_2_myst[["pca"]][["sdev"]][1]^2/sum(trajectory_means_groups_modules_2_myst[["pca"]][["sdev"]]^2)*100), digits = 2),"%)"))+ 
  ylab(paste0("PC 2 (",round(as.numeric(trajectory_means_groups_modules_2_myst[["pca"]][["sdev"]][2]^2/sum(trajectory_means_groups_modules_2_myst[["pca"]][["sdev"]]^2)*100), digits = 2),"%)"))+
  guides(alpha = guide_legend(override.aes = list(fill = NA, colour = "grey10")))+
  theme(plot.title = element_text(size = 16, face = 3, hjust = 0.5),legend.text = element_text(size = 11), legend.background = element_blank(),
        legend.key = element_blank(), legend.title = element_text(size = 12, face = "bold"), legend.position = "bottom", 
        legend.direction = "horizontal", legend.justification = c(0,0))
trajectory_means_groups_modules_2_myst_ggplot
...
#Save p-values as object
trajectory_means_groups_modules_2_myst_length <- trajectory_means_groups_modules_2_myst_MD[["pairwise.tables"]][["D"]]
trajectory_means_groups_modules_2_myst_length_p <- trajectory_means_groups_modules_2_myst_MD[["pairwise.tables"]][["P"]]
trajectory_means_groups_modules_2_myst_direction <- trajectory_means_groups_modules_2_myst_TC[["pairwise.tables"]][["angle"]]
trajectory_means_groups_modules_2_myst_direction_p <- trajectory_means_groups_modules_2_myst_TC[["pairwise.tables"]][["P"]]
trajectory_means_groups_modules_2_myst_shape <- trajectory_means_groups_modules_2_myst_SD[["pairwise.tables"]][["D"]]
trajectory_means_groups_modules_2_myst_shape_p <- trajectory_means_groups_modules_2_myst_SD[["pairwise.tables"]][["P"]]

#Make list to change tables faster
trajectory_means_groups_modules_2_myst_list <- list(trajectory_means_groups_modules_2_myst_length, trajectory_means_groups_modules_2_myst_length_p, trajectory_means_groups_modules_2_myst_direction, trajectory_means_groups_modules_2_myst_direction_p, 
                                                    trajectory_means_groups_modules_2_myst_shape, trajectory_means_groups_modules_2_myst_shape_p)
...
#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  rownames(trajectory_means_groups_modules_2_myst_list[[l]]) <- modules_2_list_factor
  colnames(trajectory_means_groups_modules_2_myst_list[[l]]) <- modules_2_list_factor
}

#Save only lower triangle for each
trajectory_means_groups_modules_2_myst_lower_tri_list <- list()

#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  trajectory_means_groups_modules_2_myst_lower_tri_list[[l]] <- get_upper_tri(trajectory_means_groups_modules_2_myst_list[[l]])
}

#Melt to make table in the format needed for heatmap
trajectory_means_groups_modules_2_myst_melt <- list()

#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  trajectory_means_groups_modules_2_myst_melt[[l]] <- melt(trajectory_means_groups_modules_2_myst_lower_tri_list[[l]], na.rm = TRUE)
}

#Create single data frames 
trajectory_means_groups_modules_2_myst_length_melt <- data.frame(trajectory_means_groups_modules_2_myst_melt[[1]], p = trajectory_means_groups_modules_2_myst_melt[[2]][[3]])
trajectory_means_groups_modules_2_myst_direction_melt <- data.frame(trajectory_means_groups_modules_2_myst_melt[[3]], p = trajectory_means_groups_modules_2_myst_melt[[4]][[3]])
trajectory_means_groups_modules_2_myst_shape_modules_2_myst_melt <- data.frame(trajectory_means_groups_modules_2_myst_melt[[5]], p = trajectory_means_groups_modules_2_myst_melt[[6]][[3]])

#Create columns where only significant values are shown
trajectory_means_groups_modules_2_myst_length_melt <- trajectory_means_groups_modules_2_myst_length_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                    p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                    value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 3)))
trajectory_means_groups_modules_2_myst_direction_melt <- trajectory_means_groups_modules_2_myst_direction_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                          p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                          value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 2)))
trajectory_means_groups_modules_2_myst_shape_modules_2_myst_melt <- trajectory_means_groups_modules_2_myst_shape_modules_2_myst_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                                                p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                                                value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 3)))
...
#Heatmaps will give error if no variables significant!!
#Check NAs first - if TRUE do not plot
all(is.na(trajectory_means_groups_modules_2_myst_length_melt$p_if_sig))

all(is.na(trajectory_means_groups_modules_2_myst_direction_melt$p_if_sig))

all(is.na(trajectory_means_groups_modules_2_myst_shape_modules_2_myst_melt$p_if_sig))

#Nice heatmap plot for each variable
trajectory_means_groups_modules_2_myst_length_heatmap_ggplot <- ggplot(data = trajectory_means_groups_modules_2_myst_length_melt, aes(Var2, Var1, fill = p_if_sig))+
  geom_tile(colour = "gray80")+
  geom_text(aes(Var2, Var1, label = value_if_sig), color = "white", size = 5) +
  ...
  guides(fill = guide_colorbar(barwidth = 6, barheight = 1,
                               title.position = "top", title.hjust = 0.5))
trajectory_means_groups_modules_2_myst_length_heatmap_ggplot

plotTmys<-ggarrange(trajectory_means_groups_modules_2_myst_length_heatmap_ggplot,
                    ncol = 1, nrow = 1, common.legend = F)
...
##Odontoceti ----
#First perform procD.lm to create linear model that describes what we are trying to test - shape changes at each stage (category) considering the 3 modules (R, B, W)
fit_shape_means_modules_2_odont_group_category <- procD.lm(as.matrix(phylo_pcscores_means_R_B_odontoceti[,c(1:14)]) ~ phylo_pcscores_means_R_B_odontoceti$module * phylo_pcscores_means_R_B_odontoceti$category, iter = 999, RRPP = F)

#Check that there is a significant correlation
summary(fit_shape_means_modules_2_odont_group_category)

#Save results to file
sink("Output/7-Trajectory means/fit_shape_means_modules_2_odont_group_category.txt")
summary(fit_shape_means_modules_2_odont_group_category)
sink() 

#Use fit to calculate trajectories
trajectory_means_groups_modules_2_odont <- trajectory.analysis(fit_shape_means_modules_2_odont_group_category, groups = phylo_pcscores_means_R_B_odontoceti$module, traj.pts = phylo_pcscores_means_R_B_odontoceti$category, 
                                                               pca = TRUE, print.progress = F) 

#View results
#Magnitude differences between trajectories, standard summary - are trajectories different in length?
trajectory_means_groups_modules_2_odont_MD <- summary(trajectory_means_groups_modules_2_odont, show.trajectories = TRUE, attribute = "MD") 
trajectory_means_groups_modules_2_odont_MD

#Trajectory correlations -  are trajectories different in angle/direction?
trajectory_means_groups_modules_2_odont_TC <- summary(trajectory_means_groups_modules_2_odont, show.trajectories = TRUE, attribute = "TC", angle.type = "deg")
trajectory_means_groups_modules_2_odont_TC 

#Trajectory shape differences - are trajectories different in shape?
trajectory_means_groups_modules_2_odont_SD <- summary(trajectory_means_groups_modules_2_odont, show.trajectories = TRUE, attribute = "SD") 
trajectory_means_groups_modules_2_odont_SD 

#Save results to file
sink("Output/7-Trajectory means/trajectory_means_groups_modules_2_odontoceti.txt")
print("Magnitude difference (absolute difference between path distances) - length")
trajectory_means_groups_modules_2_odont_MD 
print("Correlations (angles) between trajectories - direction")
trajectory_means_groups_modules_2_odont_TC
...
print("Summary tables")
trajectory_means_groups_modules_2_odont_MD$summary.table
trajectory_means_groups_modules_2_odont_TC$summary.table
trajectory_means_groups_modules_2_odont_SD$summary.table
sink() 

#Plot results - PCA of fitted values
trajectory_means_groups_modules_2_odont_plot <- plot(trajectory_means_groups_modules_2_odont, main = "Trajectories of growth by modules",  pch = c(22,24), #title and type of point to be used
                                                     col = c("darkgray","lightgray"), bg = c("darkgray","lightgray"), cex = 1, font.main = 2) #improve graphics
#Add line between groups_rostrum
add.trajectories(trajectory_means_groups_modules_2_odont_plot, 
                 traj.pch = c(22,24), traj.col = 1, traj.lty = 1, traj.lwd = 1, traj.cex = 1.5, traj.bg = 1, 
                 start.bg = "green", end.bg = "red") #trajectory line graphics
#Add legend to see which trajectory belongs to each group
...
##Make better PCA plot using ggplot
#Read PC scores as tibble
trajectory_means_groups_modules_2_odont_pcscores <- as_tibble(trajectory_means_groups_modules_2_odont_plot [["pc.points"]])

#Add group names and other attributes to tibble as columns
trajectory_means_groups_modules_2_odont_pcscores <- trajectory_means_groups_modules_2_odont_pcscores %>% mutate(specimens = phylo_pcscores_means_R_B_odontoceti$specimens, category = phylo_pcscores_means_R_B_odontoceti$category,  
                                                                                                                module = phylo_pcscores_means_R_B_odontoceti$module, size = phylo_pcscores_means_R_B_odontoceti$size)
glimpse(trajectory_means_groups_modules_2_odont_pcscores)

#Calculate means of PC1 and PC2 at each category per group to draw trajectories
trajectory_means_groups_modules_2_odont_pcscores_means <- trajectory_means_groups_modules_2_odont_pcscores %>% group_by(module, category) %>%
  summarise_at(vars(PC1, PC2), list(mean = mean))              #function for means, both columns
glimpse(trajectory_means_groups_modules_2_odont_pcscores_means)

#Rename columns so they are easier to use for plot
trajectory_means_groups_modules_2_odont_pcscores_means <- trajectory_means_groups_modules_2_odont_pcscores_means %>% rename(x = PC1_mean, y = PC2_mean)
glimpse(trajectory_means_groups_modules_2_odont_pcscores_means)

#Nice plot
trajectory_means_groups_modules_2_odont_ggplot <- ggplot(trajectory_means_groups_modules_2_odont_pcscores, aes(x = PC1, y = PC2, shape = module, group = module))+
  geom_point(aes(alpha = category), size = 2, colour = "grey10", fill = "grey10", show.legend = F)+
  geom_point(data = trajectory_means_groups_modules_2_odont_pcscores_means, aes(x = x, y = y, fill = module, shape = module, alpha = category, group = module), colour = "grey10",
             size = 6, inherit.aes = F)+
  geom_path(data = trajectory_means_groups_modules_2_odont_pcscores_means, aes(x = x, y = y, colour = module, group = module, linewidth = category), inherit.aes = F,
            linejoin = 'mitre', arrow = arrow(angle = 40, length = unit(0.02, "npc"), ends = "last", type = "open"), show.legend = F)+
  scale_alpha_manual(name = "Growth stage", labels =  levels(categories), #to be ordered as they appear in tibble
                     values = c(0.3, 0.4, 0.6, 0.8))+            #legend and color adjustments
  scale_shape_manual(name = "Modules", labels = modules_2_list_factor, values = shapes_modules_2)+
  ...
  scale_linewidth_manual(values = c(0.8, 1.2, 1.5, 1.8))+
  theme_bw()+
  xlab(paste0("PC 1 (",round(as.numeric(trajectory_means_groups_modules_2_odont[["pca"]][["sdev"]][1]^2/sum(trajectory_means_groups_modules_2_odont[["pca"]][["sdev"]]^2)*100), digits = 2),"%)"))+ 
  ylab(paste0("PC 2 (",round(as.numeric(trajectory_means_groups_modules_2_odont[["pca"]][["sdev"]][2]^2/sum(trajectory_means_groups_modules_2_odont[["pca"]][["sdev"]]^2)*100), digits = 2),"%)"))+
  guides(alpha = guide_legend(override.aes = list(fill = NA, colour = "grey10")))+
  theme(plot.title = element_text(size = 16, face = 3, hjust = 0.5),legend.text = element_text(size = 11), legend.background = element_blank(),
        legend.key = element_blank(), legend.title = element_text(size = 12, face = "bold"), legend.position = "bottom", 
        legend.direction = "horizontal", legend.justification = c(0,0))
trajectory_means_groups_modules_2_odont_ggplot
...
ggarrange(trajectory_means_groups_modules_2_myst_ggplot, trajectory_means_groups_modules_2_odont_ggplot,
          ncol = 2, nrow = 1, common.legend = T, legend = "bottom")

###Heatmaps for pairwise comparison trajectories ----

#Save p-values as object
trajectory_means_groups_modules_2_odont_length <- trajectory_means_groups_modules_2_odont_MD[["pairwise.tables"]][["D"]]
trajectory_means_groups_modules_2_odont_length_p <- trajectory_means_groups_modules_2_odont_MD[["pairwise.tables"]][["P"]]
trajectory_means_groups_modules_2_odont_direction <- trajectory_means_groups_modules_2_odont_TC[["pairwise.tables"]][["angle"]]
trajectory_means_groups_modules_2_odont_direction_p <- trajectory_means_groups_modules_2_odont_TC[["pairwise.tables"]][["P"]]
trajectory_means_groups_modules_2_odont_shape <- trajectory_means_groups_modules_2_odont_SD[["pairwise.tables"]][["D"]]
trajectory_means_groups_modules_2_odont_shape_p <- trajectory_means_groups_modules_2_odont_SD[["pairwise.tables"]][["P"]]

#Make list to change tables faster
trajectory_means_groups_modules_2_odont_list <- list(trajectory_means_groups_modules_2_odont_length, trajectory_means_groups_modules_2_odont_length_p, trajectory_means_groups_modules_2_odont_direction, trajectory_means_groups_modules_2_odont_direction_p, 
                                                     trajectory_means_groups_modules_2_odont_shape, trajectory_means_groups_modules_2_odont_shape_p)

#Set correct row and col names for both
#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  rownames(trajectory_means_groups_modules_2_odont_list[[l]]) <- modules_2_list_factor
  colnames(trajectory_means_groups_modules_2_odont_list[[l]]) <- modules_2_list_factor
}

#Save only lower triangle for each
trajectory_means_groups_modules_2_odont_lower_tri_list <- list()

#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  trajectory_means_groups_modules_2_odont_lower_tri_list[[l]] <- get_upper_tri(trajectory_means_groups_modules_2_odont_list[[l]])
}

#Melt to make table in the format needed for heatmap
trajectory_means_groups_modules_2_odont_melt <- list()

#Loop
for (l in 1:6){   #number of variable is fixed, given by parwise results
  trajectory_means_groups_modules_2_odont_melt[[l]] <- melt(trajectory_means_groups_modules_2_odont_lower_tri_list[[l]], na.rm = TRUE)
}

#Create single data frames 
trajectory_means_groups_modules_2_odont_length_melt <- data.frame(trajectory_means_groups_modules_2_odont_melt[[1]], p = trajectory_means_groups_modules_2_odont_melt[[2]][[3]])
trajectory_means_groups_modules_2_odont_direction_melt <- data.frame(trajectory_means_groups_modules_2_odont_melt[[3]], p = trajectory_means_groups_modules_2_odont_melt[[4]][[3]])
trajectory_means_groups_modules_2_odont_shape_melt <- data.frame(trajectory_means_groups_modules_2_odont_melt[[5]], p = trajectory_means_groups_modules_2_odont_melt[[6]][[3]])

#Create columns where only significant values are shown
trajectory_means_groups_modules_2_odont_length_melt <- trajectory_means_groups_modules_2_odont_length_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                      p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                      value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 3)))
trajectory_means_groups_modules_2_odont_direction_melt <- trajectory_means_groups_modules_2_odont_direction_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                            p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                            value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 2)))
trajectory_means_groups_modules_2_odont_shape_melt <- trajectory_means_groups_modules_2_odont_shape_melt %>% mutate(sig_p = ifelse(p < .05, T, F),
                                                                                                                    p_if_sig = ifelse(sig_p, p, NA),
                                                                                                                    value_if_sig = ifelse(sig_p, value, NA)) %>%
  mutate_at(vars(starts_with("value")), list(~ round(., 3)))
...
#Heatmaps will give error if no variables significant!!
#Check NAs first - if TRUE do not plot
all(is.na(trajectory_means_groups_modules_2_odont_length_melt$p_if_sig))

all(is.na(trajectory_means_groups_modules_2_odont_direction_melt$p_if_sig))

all(is.na(trajectory_means_groups_modules_2_odont_shape_melt$p_if_sig))
